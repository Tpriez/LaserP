...
public class Painter extends Activity {
    ...
    private class SaveTask extends AsyncTask<Void, Void, String> {
        private ProgressDialog dialog = ProgressDialog.show(Painter.this, getString(R.string.saving_title), getString(R.string.saving_to_sd), true);
    ...
    private class SetWallpaperTask extends AsyncTask<Void, Void, Boolean> {
        ...
        private ProgressDialog mDialog = ProgressDialog.show(Painter.this, getString(R.string.wallpaper_title), getString(R.string.aply_wallpaper), true);
        ...
        protected Boolean doInBackground(Void... none) {
            ...
            Bitmap wallpaperBitmap = Bitmap.createBitmap(wallpaperWidth, wallpaperHeight, Bitmap.Config.ARGB_8888);
            // wait bitmap
            ...
            wallpaperCanvas.drawBitmap(currentBitmap, (wallpaperWidth - currentBitmap.getWidth()) / 2, (wallpaperHeight - currentBitmap.getHeight()) / 2, null);
    ...
    protected void onResume() {
        ...
        String lang = preferences.getString(getString(R.string.preferences_language), null);
        ...
        mOpenLastFile = preferences.getBoolean(getString(R.string.preferences_last_file), true);

        mVolumeButtonsShortcuts = Integer.parseInt(preferences.getString(getString(R.string.preferences_volume_shortcuts),String.valueOf(SHORTCUTS_VOLUME_BRUSH_SIZE)));
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        switch (keyCode) {
            case KeyEvent.KEYCODE_BACK:
                if (mCanvas.isSetup()) {
                ...
                else if (mCanvas.isChanged()|| (!mIsNewFile && !new File(mSettings.lastPicture).exists())) {
                    ...
                    int beforeExit = Integer.parseInt(preferences.getString(getString(R.string.preferences_before_exit),String.valueOf(BEFORE_EXIT_SUBMIT)));
                        ...
                        if (file_uri != null) {
                            ...
                            if (picture.exists()) {
                                ...
                                if (bitmap != null) {
                                    ...
                                    int backupOption = Integer.parseInt(preferences.getString(getString(R.string.preferences_backup_openeded_file),String.valueOf(BACKUP_OPENED_ONLY_FROM_OTHER)));
                                    ...
                                    switch (backupOption) {
                                        case BACKUP_OPENED_ONLY_FROM_OTHER:
                                            if (!picture.getParentFile().getName().equals(getString(R.string.app_name))) {
                                                pictureName = FileSystem.copyFile(picture.getAbsolutePath(),getSaveDir()+ picture.getName());
                                            } else {
    ...
    public void setPreset(View v) {
        switch (v.getId()) {
            case R.id.preset_pencil:
                mCanvas.setPreset(new BrushPreset(BrushPreset.PENCIL, mCanvas.getCurrentPreset().color));
                break;
            case R.id.preset_brush:
                mCanvas.setPreset(new BrushPreset(BrushPreset.BRUSH, mCanvas.getCurrentPreset().color));
                break;
            case R.id.preset_marker:
                mCanvas.setPreset(new BrushPreset(BrushPreset.MARKER, mCanvas.getCurrentPreset().color));
                break;
            case R.id.preset_pen:
                mCanvas.setPreset(new BrushPreset(BrushPreset.PEN, mCanvas.getCurrentPreset().color));
                break;
        }
...
//        mSettings.orientation = settings.getInt(getString(R.string.settings_orientation),ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
//
